...
class MCDODE():
  ...
  def _run_simulation(self, f_car, f_truck, counter = 0):
      hash1 = hashlib.sha1()
      hash1.update((str(time.time()) + str(0)).encode('utf-8'))
  ...
  def _massage_raw_dar(self, raw_dar, ass_freq, f, num_assign_interval):
    ...
    link_seq = (np.array([self.observed_links.index(x) for x in raw_dar[:, 2].astype(int)])
                    + raw_dar[:, 3] * num_e_link / ass_freq).astype((int))
    path_seq = (raw_dar[:, 0] + (raw_dar[:, 1] / small_assign_freq).astype(int) * num_e_path).astype(int)
    # print path_seq
  ...
  def _compute_grad_on_car_link_tt(self, dta, one_data_dict):
    ...
    tt_e = dta.get_car_link_tt(np.arange(0, self.num_loading_interval, self.ass_freq, dtype=np.float64), True).flatten(order='F')
  ...
  def _compute_grad_on_truck_link_tt(self, dta, one_data_dict):
    ...
    tt_e = dta.get_truck_link_tt(np.arange(0, self.num_loading_interval, self.ass_freq, dtype=np.float64), True).flatten(order='F')
  ...
  def _get_loss(self, one_data_dict, dta):
    ...
    if self.config['use_car_link_tt'] or self.config['compute_car_link_tt_loss']:
      ...
      x_tt_e = dta.get_car_link_tt(np.arange(0, self.num_loading_interval, self.ass_freq, dtype=np.float64), True).flatten(order='F')
    ...
    if self.config['use_truck_link_tt'] or self.config['compute_truck_link_tt_loss']:
      ...
      x_tt_e = dta.get_truck_link_tt(np.arange(0, self.num_loading_interval, self.ass_freq, dtype=np.float64), True).flatten(order='F')
