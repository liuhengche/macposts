...
class MNM_demand():
  ...
  def add_demand(self, O, D, car_demand, truck_demand, overwriting = False):
    ...
    if (not overwriting) and (D in self.demand_dict[O].keys()):
      print("Error, exists OD demand already") 
    else:
...
class MNM_network_builder():
  ...
  def dump_to_folder(self, path, config_file_name = 'config.conf',
    ...
    # f.close()
    with open(os.path.join(path, link_file_name), 'w') as f:  # Open in text mode
      f.write(self.generate_link_text())  

    # f = open(os.path.join(path, node_file_name), 'wb')
    # f.write(self.generate_node_text())
    # f.close()
    with open(os.path.join(path, node_file_name), 'w') as f:  # Open in text mode
      f.write(self.generate_node_text())  

    # f = open(os.path.join(path, config_file_name), 'wb')
    # f.write(str(self.config))
    # f.close()
    with open(os.path.join(path, config_file_name), 'w') as f:  # Open in text mode
      f.write(str(self.config)) 
    ...
    with open(os.path.join(path, od_file_name), 'w') as f_od:
        f_od.write(self.od.generate_text())

    with open(os.path.join(path, demand_file_name), 'w') as f_demand:
        f_demand.write(self.demand.generate_text())

    with open(os.path.join(path, graph_file_name), 'w') as f_graph:
        f_graph.write(self.graph.generate_text())

    with open(os.path.join(path, pathtable_file_name), 'w') as f_pathtable:
        f_pathtable.write(self.path_table.generate_table_text())
    ...
    if self.route_choice_flag:
      ...
      # f.close()
      with open(os.path.join(path, path_p_file_name), 'w') as f:
        f.write(self.path_table.generate_portion_text())
