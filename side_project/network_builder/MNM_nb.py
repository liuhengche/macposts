...
class MNM_config():
  def __init__(self):
    ...
    self.type_dict = {'network_name' : str, 'unit_time': int, 'total_interval': int,
                      'assign_frq' : int, 'start_assign_interval': int, 'max_interval': int,
                      'flow_scalar': int, 'num_of_link': int, 'num_of_node': int, 
                      'num_of_O': int, 'num_of_D': int, 'OD_pair': int,
                      'routing_type' : str, 'rec_mode': str, 'rec_mode_para': str, 'rec_folder': str,
                      'rec_volume': int, 'volume_load_automatic_rec': int, 'volume_record_automatic_rec': int,
                      'rec_tt': int, 'tt_load_automatic_rec':int, 'tt_record_automatic_rec':int,
                      'route_frq': int, 'path_file_name': str, 'num_path': int,
                      'choice_portion': str, 'route_frq': int, 'buffer_length':int}
